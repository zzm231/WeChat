...
public class LQRRecordProgress extends View {
    ...
    public LQRRecordProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RecorderProgress, defStyleAttr, 0);
        //默认最长录制时间是10秒
        mRecordTime = a.getInteger(R.styleable.LORRecordProgress_rp_record_time, 10) * 1000;
        //默认进度条是黑色
        mProgressColor = a.getColor(R.styleable.LORRecordProgress_rp_progress_color, Color.BLACK);
        a.recycle();
